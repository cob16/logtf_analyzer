...
import begin
from logtf_analyser.rest_actions import RestActions
...
def send(message: 'set the message body' = None, *feeds: 'Bare list of feeds {0}'.format(KNOWN_FEEDS)):
    """Sends a new logtf_analyser to the server"""
    ...
    puts(colored.green('Successfully sent logtf_analyser!'))
...
def validate_feeds(feeds):
    feeds = set(feeds)
    if not feeds:
        exit_and_fail("You must specify feed name/s ('logtf_analyser send -h' for more info)")
...
@begin.subcommand
def show(id: 'logtf_analyser id' = None):
    """Show all detail of a logtf_analyser"""
    parent = begin.context.last_return
...
def get_url(interactive):
    ...
    if config:
        if not url_valid(config):
            exit_and_fail("Url in config is not valid! use 'logtf_analyser configure --delete' to fix this")
        return config
    elif interactive:
        return url_prompt()
